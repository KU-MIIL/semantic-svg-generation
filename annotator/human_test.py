"""Human editability study webui (rebuttal Q4/S3) — MOVE / REMOVE tasks.

Participants replicate a GT-defined edit on a GENERATED SVG (two conditions:
ours = semantically grouped, vtracer = flat autotrace), while the tool
measures time. Success is NOT judged here — every result (edited SVG + PNG +
timing + action log) is saved for the authors to rate afterwards.

Design:
  - Trials: task block (move → remove) × 15 stems × 2 conditions = 60.
    Condition order per (participant, task, stem) is counterbalanced by hash;
    the UI never shows which system produced the SVG.
  - Task spec shown as GT before → after images (left panel); for MOVE the
    participant eyeballs the destination from the after image (success is
    rated semantically by the authors afterwards, so pixel-exact placement
    is not required).
  - Selection: click = single path, Ctrl+click = add/remove,
    DOUBLE-click = the whole <g> group containing that path (ours has
    semantic groups; vtracer has none, so double-click stays path-level).
  - Del removes the selection (remove task), drag moves it (move task).
  - Timer runs from trial display to "완료"; first-interaction time also
    recorded. "포기" saves the state with gave_up=true.

Results: SSemBench/curated/human_test_results/<participant>/
             <task>__<stem>__<cond>.svg / .png / .json

Run:
    .venv/bin/python human_test.py     # http://localhost:8098
"""
...
GT_SVG = CURATED / "svg"
EDIT_GT = CURATED / "edit_gt"
COND_DIRS = {"ours": CURATED / "svg_ours_all", "vtracer": CURATED / "svg_vtracer_all"}
RESULTS = CURATED / "human_test_results"
...
def cond_order(participant: str, task: str, stem: str) -> list[str]:
    ...
    return ["ours", "vtracer"] if int(h, 16) % 2 == 0 else ["vtracer", "ours"]
...
def api_trial(participant: str, idx: int) -> JSONResponse:
    ...
    t = trials[idx]
    stim_p = COND_DIRS[t["cond"]] / f"{t['stem']}.svg"
...
<div id="login">
  ...
  ⚠ 편집할 그림은 목표 그림과 세부 모양·색이 조금 다를 수 있습니다.
  <b>같은 종류의 변경</b>(같은 대상을 이동/제거)을 적용하면 됩니다.</p>
...
<main>
  ...
  <div class="panel">
    ...
    <div class="hint">클릭 = 조각 선택 ·
      <span class="kbd">Ctrl</span>+클릭 = 누적 선택 · <span class="kbd">Esc</span> = 선택 해제 ·
      <span class="kbd">Ctrl+Z</span> = 되돌리기 · 오른쪽 목록의 항목을 누르면 관련 조각이 한번에 선택됩니다</div>
  </div>
  <div class="panel">
    <h3>구성 요소 목록</h3>
    <div id="treeBox"></div>
...
// 우측 패널: 스티뮬러스 svg의 <g> 구조에서 시맨틱 트리를 만든다.
// (ours는 <g id="p1" data-label="...">를 갖고, vtracer는 그룹이 없어 빈 목록 —
//  이 차이 자체가 실험 조건이다.)
...
if __name__ == "__main__":
    ...
    print(f"Human test: move {n_move} × 2 + remove {n_remove} × 2 = {2*(n_move+n_remove)} trials/participant")
    print(f"  conditions: {list(COND_DIRS)}")
