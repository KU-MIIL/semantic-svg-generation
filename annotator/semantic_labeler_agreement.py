"""Hierarchical SVG semantic labeler — inter-annotator AGREEMENT round.

Rebuttal workflow: both annotators independently revise the SAME 88
main-paper samples (SSemBench/curated/svg). Each annotator gets a full
private copy — 박세환 edits curated/svg_sehwan, 김태훈 edits
curated/svg_taehoon — pre-loaded with the current tree.json labels.
Change what you disagree with and Save; leave the rest untouched.
Agreement is computed afterwards by comparing the two copies' tree.jsons.

Run:
    .venv/bin/python semantic_labeler_agreement.py
    # http://localhost:8096 — pick your name, ALL 88 files are yours
...
UX:
  ...
  - "Save" writes the sidecar <name>.tree.json with the full tree.
"""
...
# Agreement round: each annotator works on a PRIVATE full copy of the same
# 88 main-paper samples. No file splitting — the whole point is that both
# people label everything so the two copies can be compared.
USER_DIRS = {
    "박세환": _PKG_ROOT / "SSemBench" / "curated" / "svg_sehwan",
    "김태훈": _PKG_ROOT / "SSemBench" / "curated" / "svg_taehoon",
}
SHAPE_TAGS = ("path", "rect", "circle", "ellipse", "polygon", "polyline", "line")


# -----------------------------------------------------------------------------
# File scanning / per-user routing
# -----------------------------------------------------------------------------
...
def user_files(user: str) -> list[Path]:
    """ALL 88 files from that user's private copy. Empty for unknown user."""
    d = USER_DIRS.get((user or "").strip())
    if d is None or not d.exists():
        return []
    return sorted(d.glob("*.svg"), key=lambda p: p.stem)


def list_files() -> list[Path]:
    """Union across users (used only by the startup banner)."""
    out: list[Path] = []
...
if __name__ == "__main__":
    print("Agreement labeler — private full copies per annotator:")
    for u, d in USER_DIRS.items():
        print(f"  {u}: {len(user_files(u))} files  ({d})")
    print("Open http://localhost:8096")
