"""Hierarchical SVG semantic labeler — EMNLP curation set.

Labels the human-curated SVGs in curation_pool/renders_emnlp (filenames like
"emojitwo__svg_1f3a2.svg" — NOT digit stems, unlike the original
semantic_labeler.py which this file is a port of).

Run:
    .venv/bin/python semantic_labeler_emnlp.py
    # serves all *.svg under curation_pool/renders_emnlp at http://localhost:8088
...
SVG_DIRS = [
    _PKG_ROOT / "camera-ready-260824" / "svg",
]
...
def list_files() -> list[Path]:
    out: list[Path] = []
    for d in SVG_DIRS:
        # Curated ids like "emojitwo__svg_1f3a2" — plain name sort, no digit
        # filter (that was for the sample_full "183_grouped.svg" helper files).
        out.extend(sorted(d.glob("*.svg"), key=lambda p: p.stem))
    return out
...
# -----------------------------------------------------------------------------
_ASSIGN_PATH = _PKG_ROOT / "curation_pool" / "_assignments_emnlp_flat.json"
...
async def api_treevis(body: dict) -> Response:
    ...
    svg = files[idx]
    text = svg.read_text()
    # `saved: true` forces the on-disk tree.json; otherwise use the
    # in-progress tree the browser sent (falling back to disk if absent).
    if body.get("saved"):
        tree = load_tree(svg)
        _tree_mode = "SAVED"
    else:
        tree = body.get("tree") or load_tree(svg)
        _tree_mode = "LIVE" if body.get("tree") else "SAVED"
    ...
    if isinstance(tree, dict) and tree.get("fail"):
        ...
        draw = ImageDraw.Draw(canvas)
        title = f"{svg.parent.parent.name}/{svg.name}  —  {len(root.paths)} paths  [{_tree_mode}]"
...
<header>
  ...
  <button onclick="showTreeVis(false)" title="Render the CURRENT in-progress (unsaved) tree — for debugging while labeling">🌳 Tree</button>
  <button onclick="showTreeVis(true)" title="Render the SAVED tree.json from disk — the finalized result">🌳 Saved</button>
...
let files = [];
let curFile = 0;
let curUser = localStorage.getItem('labeler_user') || 'all';
...
async function showTreeVis(useSaved) {
  const overlay = document.getElementById('treevisOverlay');
  const content = document.getElementById('treevisContent');
  content.innerHTML = '<i style="color:#888;">rendering ' + (useSaved ? 'saved' : 'live') + ' tree…</i>';
  overlay.style.display = 'flex';
  try {
    // useSaved: render the on-disk tree.json (finalized result).
    // otherwise: render the current in-progress state.tree (unsaved, for debugging).
    const payload = useSaved
      ? {idx: curFile, user: curUser, saved: true}
      : {idx: curFile, user: curUser, tree: state.tree};
