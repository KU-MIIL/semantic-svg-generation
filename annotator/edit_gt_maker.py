"""GT edit-dataset maker for the human editability study (rebuttal Q4/S3).

Loads the 88 main-paper samples (SSemBench/curated/svg + tree.json) and lets
the author create ground-truth edited versions per sample for three edit
types — move / remove / recolor. Each edit starts from the ORIGINAL svg
(edits are independent, not cumulative).

UI:
  - Center canvas: interactive SVG. Click = select a single path,
    Ctrl+click = add/remove from selection, Esc = clear selection.
  - Right panel: the sample's GT semantic tree (from tree.json). Clicking a
    node selects ALL paths under that node at once; Ctrl+click adds.
  - move    — drag any selected element; the whole selection translates.
  - remove  — Delete button (or Del key) removes the selection.
  - recolor — rainbow-7 palette; clicking a swatch recolors the selection.
  - Save writes to SSemBench/curated/edit_gt/:
        <stem>__<edit>.svg   edited SVG (selection markup stripped)
        <stem>__<edit>.png   512px render (server-side, cairosvg)
        <stem>__<edit>.json  action log: which paths/tree node, dx/dy or color

Run:
    .venv/bin/python edit_gt_maker.py     # http://localhost:8097
"""
...
SRC = _PKG_ROOT / "SSemBench" / "curated" / "svg"
OUT = _PKG_ROOT / "SSemBench" / "curated" / "edit_gt"
...
def load_tree(svg: Path) -> dict:
    p = svg.with_suffix(".tree.json")
    try:
        return json.loads(p.read_text(encoding="utf-8"))
    except Exception:
        return {"paths": [], "root": {"children": []}}
...
function resetCanvas() {
  ...
  applyClipping();
  refreshSelUI();
  if (state.tree?.paths && state.tree.paths.length !== shapes.length) {
    toast(`주의: tree.json paths ${state.tree.paths.length}개 ≠ 화면 ${shapes.length}개 — 어긋난 인덱스는 무시됩니다`);
  }
}
...
function leafPathsOf(node) {
  if (!node.children) return (node.paths || []).slice();
  let out = [];
  for (const c of node.children) out = out.concat(leafPathsOf(c));
  if (!out.length) out = (node.paths || []).slice();
  return out;
}
...
function renderTree() {
  const box = $("treeBox");
  box.innerHTML = "";
  const rootBody = state.tree.root || state.tree;
  const mk = (node, depth, labelPath) => {
    const div = document.createElement("div");
    div.className = "tnode";
    div.style.paddingLeft = (6 + depth * 16) + "px";
    const isLeaf = !node.children;
    const lbl = node.label || (depth === 0 ? "root" : "(unnamed)");
    const myPath = labelPath ? labelPath + " > " + lbl : lbl;
    const paths = depth === 0 ? (state.tree.paths || leafPathsOf(rootBody)) : leafPathsOf(node);
    div.innerHTML = `<span class="chip ${isLeaf ? "leaf" : "grp"}">${isLeaf ? "leaf" : "grp"}</span>` +
      `<strong>${lbl}</strong><span class="cnt">${paths.length}p</span>`;
    div.onclick = (e) => {
      setSelection(paths.filter(i => shapes[i]), e.ctrlKey || e.metaKey);
      activeTreeNode = myPath;
      markTreeActive(div);
    };
    box.appendChild(div);
    for (const c of (node.children || [])) mk(c, depth + 1, myPath);
  };
  mk(rootBody, 0, "");
}
